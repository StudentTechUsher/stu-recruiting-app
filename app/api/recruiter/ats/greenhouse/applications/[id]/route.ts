...
import { fetchGreenhouseApplication } from "@/lib/ats/greenhouse";
...
export async function GET(
  ...
  try {
    const result = await fetchGreenhouseApplication(context.org_id, id);
    if (!result) return badRequest("Application not found");
    return ok(result);
  } catch (e) {
